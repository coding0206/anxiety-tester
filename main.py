...
streamer = ThreadedStreamer(inference.only_inference, batch_size=64, max_latency=0.15)
...
def predict(sentence_list, phoneme_sequence_list, retry):
    ...
    if retry == 2 and len(sentence_list[0]) <= 40:
        ...
        for corrected_sentence in corrected_sentence_list:
            corrected_perpelxity_list.append(trigram_lm.perplexity(ws([corrected_sentence])[0]))
...
if __name__ == "__main__":
    ...
    app.run(host='0.0.0.0', port=options.port, debug=options.debug, threaded=True, processes=True)
